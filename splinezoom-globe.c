...
void spline_row(UCHAR *c,int nc,UCHAR *out,int len)
{
 int i,it;
 float t,max;
 max = nc;
 for (i=0; i<len; i++) {
	...
	it = t;
	t -= it;
	out[i] = interp(c+it+2,t);
	}
}

zoom(UCHAR *in,UCHAR *out,int xdim,int ydim,int outx,int outy)
{
 UCHAR *tmp,*icol,*ocol,*row,*irow;
 int x,y;
 tmp = arralloc(UCHAR,outx*ydim);
 irow = arralloc(UCHAR,xdim+6);
 for (y=0; y<ydim; y++) {
	row = in+y*xdim;
	irow[0] = row[xdim-3];
	irow[1] = row[xdim-2];
	irow[2] = row[xdim-1];
	for (x=0; x<xdim; x++) irow[x+3] = row[x];
	irow[xdim+3] = row[0];
	irow[xdim+4] = row[1];
	irow[xdim+5] = row[2];
	spline_row(irow,xdim,tmp+y*outx,outx);
	}
 free(irow);
